...
async function deleteProductImages(keys) {
    if (!keys.length) {
        return;
    }
...
async function deleteProduct(req, res) {
    ...
    await userModel.updateMany({ "products.product": deletedProduct._id }, { $pull: { products: { product: deletedProduct._id } } });
...
async function updateProduct(req, res) {
    ...
    try {
        ...
        if (title !== undefined || price !== undefined) {
            try {
                await userModel.updateMany({ "products.product": updatedProduct._id }, {
                    $set: {
